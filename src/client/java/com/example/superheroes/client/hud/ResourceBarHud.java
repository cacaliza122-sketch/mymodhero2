...
public final class ResourceBarHud {
	...
	private static final int HERO_BADGE_SIZE = 24;
	...
	public static void render(GuiGraphics graphics, DeltaTracker tracker) {
		...
		HudUtil.roundedRectGradient(graphics, X, Y, PANEL_WIDTH, PANEL_HEIGHT, theme.panelTop(), theme.panelBottom());
		HudUtil.roundedRectBorder(graphics, X, Y, PANEL_WIDTH, PANEL_HEIGHT, theme.panelBorder());
		...
		if (heroId != null && heroId.equals(IronManHero.ID)) {
			drawHeroBadge(graphics, mc, theme);
		}
	}

	private static void drawHeroBadge(GuiGraphics g, Minecraft mc, HeroTheme theme) {
		int bx = X + PANEL_WIDTH - HERO_BADGE_SIZE - 8;
		int by = Y + 4;
		HudUtil.roundedRectFill(g, bx, by, HERO_BADGE_SIZE, HERO_BADGE_SIZE, 0xFF1A0204);
		...
		g.fill(cx - 4, cy - 6, cx + 4, cy - 4, theme.manaIcon());
		g.fill(cx - 5, cy - 4, cx - 3, cy + 1, theme.manaIcon());
		g.fill(cx + 3, cy - 4, cx + 5, cy + 1, theme.manaIcon());
		g.fill(cx - 2, cy - 2, cx - 1, cy, theme.energyDark());
		g.fill(cx + 1, cy - 2, cx + 2, cy, theme.energyDark());
		g.fill(cx - 4, cy + 2, cx + 4, cy + 4, theme.energyIcon());
		g.fill(cx - 1, cy + 4, cx + 1, cy + 6, theme.energyIcon());
	}
